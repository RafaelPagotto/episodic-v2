...
import { getUserLibraryShows } from "../features/library/data";
...
describe("library data loading", () => {
  it("shows a completed miniseries when all season 1 episodes are watched", async () => {
    ...
    addShow(db, {
      episodes,
      show: showRow(100, "The Fall of the House of Usher", "Ended"),
      watchedEpisodes: episodes.map((episode) =>
        watchedEpisodeRow(episode.show_tmdb_id, episode.season_number, episode.episode_number),
      ),
    });

    const shows = await getUserLibraryShows(client(db), USER_ID);
  ...
  it("does not show a fully watched returning show as zero progress", async () => {
    const db = new FakeSupabase();
    const episodes = Array.from({ length: 84 }, (_, index) => episodeRow(200, 1, index + 1));

    addShow(db, {
      episodes,
      show: showRow(200, "Rick and Morty", "Returning Series"),
      watchedEpisodes: episodes.map((episode) =>
        watchedEpisodeRow(episode.show_tmdb_id, episode.season_number, episode.episode_number),
      ),
    });
    ...
    expect(shows[0]).toMatchObject({
      ...
      watchedEpisodeCount: 84,
    });
  });

  it("counts a show whose episode and watched rows appear after a pagination boundary", async () => {
    const db = new FakeSupabase();
    const pageFillerEpisodes = Array.from({ length: MULTI_SHOW_EPISODE_PAGE_SIZE }, (_, index) =>
      episodeRow(1, 1, index + 1),
    );
    const boundaryEpisodes = Array.from({ length: 8 }, (_, index) => episodeRow(2, 1, index + 1));

    addShow(db, {
      addedAt: "2026-02-01T00:00:00.000Z",
      episodes: pageFillerEpisodes,
      show: showRow(1, "Long-running filler", "Returning Series"),
      watchedEpisodes: pageFillerEpisodes.map((episode) =>
        watchedEpisodeRow(episode.show_tmdb_id, episode.season_number, episode.episode_number),
      ),
    });
    addShow(db, {
      addedAt: "2026-02-02T00:00:00.000Z",
      episodes: boundaryEpisodes,
      show: showRow(2, "After Boundary", "Ended"),
      watchedEpisodes: boundaryEpisodes.map((episode) =>
        watchedEpisodeRow(episode.show_tmdb_id, episode.season_number, episode.episode_number),
      ),
    });

    const shows = await getUserLibraryShows(client(db), USER_ID);
    const boundaryShow = shows.find((show) => show.tmdbId === 2);

    expect(boundaryShow).toMatchObject({
      displayStatus: "completed",
      progressPercentage: 100,
      totalEpisodeCount: 8,
      watchedEpisodeCount: 8,
  ...
  it("excludes Specials from Library main progress", async () => {
    const db = new FakeSupabase();

    addShow(db, {
      episodes: [episodeRow(300, 0, 1), episodeRow(300, 1, 1), episodeRow(300, 1, 2)],
      show: showRow(300, "Specials Example", "Ended"),
      watchedEpisodes: [watchedEpisodeRow(300, 1, 1), watchedEpisodeRow(300, 1, 2)],
    });
